...
write_fleetpy_constant_config_file(os.path.join(FLEETPY_SCENARIOS_PATH, "constant_config.csv"),fleetpy_constant_config,sim_start_time,sim_end_time)
...
for i in range(num_iterations):
    ...
    for r in range(num_replications):
        ...
        #save all travelers
        df.to_csv(os.path.join(WORKSPACE_PATH, f"i{i:03}_r{r:03}_all_presim_demand.csv"),index=False)
        requests.append(df)
        iter_demand_modesplit.append(get_mode_stats(df))

        #save nsm demand
        rqfile = f"i{i:03}_r{r:03}_nsm.csv"
        df[df["choice"]==4].to_csv(os.path.join(FLEETPY_DEMAND_PATH, rqfile),index=False)
    ...
    write_fleetpy_scenario_config_file(os.path.join(FLEETPY_SCENARIOS_PATH, "scenario_config.csv"),i,num_replications,scenario_basename,fleetpy_params) # inner_loop_args)
    command = [sys.executable, os.path.join(FLEETPY_PATH, "run_private_jerusalem.py")]
    ...
    for r in range(num_replications):
        ...
        df.to_csv(os.path.join(WORKSPACE_PATH, f"i{i:03}_r{r:03}_all_postsim_demand.csv"),index=False)
...
with open(os.path.join(RESULT_PATH, f"{config_name}_results.csv"), mode="w", newline="") as file:
...
nsmstats_df.to_csv(os.path.join(RESULT_PATH, f"{config_name}_nsmstats.csv"),index=False)
choicestats_df = choicestats.to_ggplot()
choicestats_df.to_csv(os.path.join(RESULT_PATH, f"{config_name}_choicestats.csv"),index=False)
...
modesplit_df.to_csv(os.path.join(RESULT_PATH, f"{config_name}_modesplit.csv"),index=False)
