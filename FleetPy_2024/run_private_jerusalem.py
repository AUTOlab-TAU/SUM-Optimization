...
def run_single_simulation(scenario_parameters):
    print("printing scenario_parameters")
...
def run_scenarios(constant_config_file, scenario_file, n_parallel_sim=30, n_cpu_per_sim=1, evaluate=1, log_level="info",
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        scs_path = os.path.join(os.path.dirname(__file__), "studies", "jerusalem", "scenarios")
        ...
        sc = os.path.join(scs_path, "scenario_config.csv")
        #sc = os.path.join(scs_path, "scenario_fleetpy_test_config.csv")

        #EXAMPLE
        #cc = os.path.join(scs_path, "constant_config_pool.csv")
        #sc = os.path.join(scs_path, "example_pool.csv")

        run_scenarios(cc, sc, log_level=log_level, n_cpu_per_sim=1, n_parallel_sim=30)
        #list_results = read_outputs_for_comparison(cc, sc)
        #all_scenario_assert_dict = {0: {"number users": 238}}
